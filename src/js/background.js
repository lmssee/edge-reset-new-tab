 ...
 *  */
chrome.runtime.onMessage.addListener((response, sender, resultReturn) => {
  /// 非礼勿视
  ...
  if (
    ...
    getLocalRefreshList(
      (result) =>
        // 收到页面的问询消息后从
        (result.refreshPageList && result.refreshPageList[id]) !== undefined &&
        ((type === "askRefresh" &&
          (message.refresh(result.refreshPageList[id]), !0)) ||
          (type === "cancelRefresh" &&
            (delete result.refreshPageList[id],
            chromeLocalStorage.set({ refreshPageList: result.refreshPageList }),
            !0)) ||
 ...
 *  获取本地储存的定时刷新页面的值
 */
function getLocalRefreshList(callBack) {
